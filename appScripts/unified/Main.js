...
function handleBunkerCredit(data, request) {
  ...
  if (!Authorization.canAccessResource(payload, null, 'write')) return createResponse(403, 'Insufficient permissions', null);
  try {
    var result = bunker_credit({ unit: data.unit, items: data.items, by: data.by, warehouse: data.warehouse });
...
function handleBunkerShatsalReport(data, request) {
  var payload = JWTUtil.verify(data.token, CONFIG.JWT_SECRET);
  if (!payload) return createResponse(401, 'Invalid or expired token', null);
  if (!Authorization.canAccessResource(payload, null, 'write'))
    return createResponse(403, 'Insufficient permissions', null);
...
function handleBunkerGetShatsal(data, request) {
  var payload = JWTUtil.verify(data.token, CONFIG.JWT_SECRET);
  if (!payload) return createResponse(401, 'Invalid or expired token', null);
  try {
...
function handleBunkerRebuildSchema(data, request) {
  var payload = JWTUtil.verify(data.token, CONFIG.JWT_SECRET);
  if (!payload) return createResponse(401, 'Invalid or expired token', null);
  try {
    var result = bunker_rebuildSchema();
    return result.success ? createResponse(200, 'ok', result)
                          : createResponse(500, result.error, null);
  } catch (e) { return createResponse(500, 'Server error: ' + e.toString(), null); }
}

function handleBunkerFixShatsalDates(data, request) {
  var payload = JWTUtil.verify(data.token, CONFIG.JWT_SECRET);
  if (!payload) return createResponse(401, 'Invalid or expired token', null);
  try {
...
function handleApsnautGetSoldierItems(data, request) {
  var payload = JWTUtil.verify(data.token, CONFIG.JWT_SECRET);
  if (!payload) return createResponse(401, 'Invalid or expired token', null);
  try {
...
function handleInspectionsGetUnitData(data, request) {
  var payload = JWTUtil.verify(data.token, CONFIG.JWT_SECRET);
  if (!payload) return createResponse(401, 'Invalid or expired token', null);
  try {
    if (!data.unit) return createResponse(400, 'חסרת מסגרת', null);
    var result = inspections_getUnitData(data.unit);
    return result.success ? createResponse(200, 'ok', result.data) : createResponse(500, result.error, null);
  } catch (e) { return createResponse(500, 'Server error: ' + e.toString(), null); }
}
...
function handleInspectionsMark(data, request) {
  ...
  try {
    ...
    var result = inspections_markItem(
      ...
      data.unit, data.team || '',
      data.itemKey, data.itemLabel || '', data.serial || '', data.type
    );
    if (result.success) {
